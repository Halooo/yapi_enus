        ...
        <Layout>
          <Content style={{ height: '100%', margin: '0 24px 0 16px', overflow: 'initial', backgroundColor: '#fff' }}>
            <Tabs type="card"  className="m-tab tabs-large" style={{ height: '100%' }}>
              <TabPane tab="Project list" key="1"><ProjectList /></TabPane>
              {this.props.currGroup.type === 'public' ? <TabPane tab="Member list" key="2"><MemberList /></TabPane> : null}
              {["admin", "owner", "guest", "dev"].indexOf(this.props.curUserRoleInGroup) > -1 || this.props.curUserRole === "admin" ? <TabPane tab="Group activity" key="3"><GroupLog /></TabPane> : ""}
              {(this.props.curUserRole === "admin" || this.props.curUserRoleInGroup === 'owner') && this.props.currGroup.type !== 'private' ? <TabPane tab="Group settings" key="4"><GroupSetting /></TabPane> : null}
            </Tabs>
          </Content>
        </Layout>
      </Layout>
