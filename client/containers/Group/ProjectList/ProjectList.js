  ...
  render() {
    ...
    const Follow = () => {
      return followProject.length ? <Row>
        <h3 className="owner-type">My Favorite</h3>
        {
    ...
    const NoFollow = () => {
      return noFollow.length ? <Row style={{ borderBottom: '1px solid #eee', marginBottom: '15px' }}>
        <h3 className="owner-type">My project</h3>
        {
    ...
    return (
      <div style={{ paddingTop: '24px' }} className="m-panel card-panel card-panel-s project-list" >
        <Row className="project-list-header">
          ...
          <Col span={8}>
            {isShow?
              <Link to="/add-project"><Button type="primary">Add project</Button></Link> :
              <Tooltip title="You do not have privilege to add project, please contact admin or group owner">
                <Button type="primary" disabled >Add project</Button>
              </Tooltip>}
          </Col>
        </Row>
        <Row>
