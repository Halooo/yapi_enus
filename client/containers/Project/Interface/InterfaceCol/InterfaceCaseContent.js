    ...
    return (
      <div style={{ padding: '6px 0' }} className="case-content">
        <div className="case-title">
          {!isEditingCasename && <Tooltip title="Click to edit" placement="bottom"><div className="case-name" onClick={this.triggerEditCasename}>
            {currCase.casename}
          </div></Tooltip>}
          ...
          <span className="inter-link" style={{ margin: '0px 8px 0px 6px', fontSize: 12 }}>
            <Link className="text" to={`/project/${currProject._id}/interface/api/${currCase.interface_id}`}>Related API</Link>
          </span>
        </div>
        <div>
          {Object.keys(currCase).length > 0 &&
            <Postman data={data} type="case" saveTip="Update and save changes" save={this.updateCase} ref={this.savePostmanRef} />
          }
        </div>
      </div>
    )
