...
export default class InterfaceColMenu extends Component {
  ...
  addorEditCol = async () => {
    ...
    if (!res.data.errcode) {
      ...
      message.success(colModalType === 'edit' ? 'Changed' : 'Added');
  ...
  showDelColConfirm = (colId) => {
    ...
    confirm({
      ...
      content: 'Note：this deletion will delete all test cases under this collection, and not revertible',
      okText:"Confirm",
      cancelText:"Cancel",
      async onOk() {
  ...
  copyInterface = async (item) => {
    ...
    message.success('Copied Collection')

  }

  showNoDelColConfirm = () => {
    confirm({
      title: 'This is the last collection',
      content: 'Note: we suggest keeping this'
    });
  }
  showDelCaseConfirm = (caseId) => {
    ...
    const params = this.props.match.params;
    confirm({
      title: 'Are you sure to delete the test case',
      content: 'Note：deleting test cases is not revertible',
      okText:"Confirm",
      cancelText:"Cancel",
      async onOk() {
  ...
  handleImportOk = async () => {
    ...
    if (!res.data.errcode) {
      ...
      message.success('Collection imported successful');
      // await this.props.fetchInterfaceColList(project_id);
      this.getList()
