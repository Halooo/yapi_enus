...
class ModalPostman extends Component {
  ...
  render() {
    ...
    return (
      <Modal
        title={<p><Icon type="edit" /> Advanced Parameter Settings</p>}
        visible={visible}
      ...
      >
              ...
              return item.type === 'dataSource' ?
                <Col span={8} className="modal-postman-col" key={index}>
                  <Collapse className="modal-postman-collapse" activeKey={this.state.activeKey} onChange={this.handleCollapse} bordered={false} accordion>
                    <Panel header={<h3 className="mock-title">Constant</h3>} key="1">
                      <Input placeholder="basic parameter" value={constantInput} onChange={(e) => this.handleConstantsInput(e.target.value, index)} />
                    </Panel>
                    <Panel header={<h3 className="mock-title">mock data</h3>} key="2">
                      <MockList click={this.mockClick(index)} clickValue={item.name}></MockList>
                    </Panel>
                    {
                      envType === 'case' && <Panel
                        header={<h3 className="mock-title">Parameters&nbsp;<Tooltip placement="top" title="YApi provides variable parameter, you can test the API with previously 'returned values' as 'parameters' of the current API to test (this is designed for one API relied on another API)"><Icon type="question-circle-o" /></Tooltip></h3>}
                        key="3">
        ...
        <Row className="modal-postman-expression">
          <Col span={6}><h3 className="title">Postman Expression</h3></Col>
          <Col span={18}>
            <span className="expression-item">{outputParams()}</span>
          </Col>
        </Row>
        <Row className="modal-postman-preview">
          <Col span={6}><h3 className="title">Preview</h3></Col>
          <Col span={18}>
            <h3>
