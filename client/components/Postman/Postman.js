...
const InsertCodeMap = [
  {
    ...
    title: "assert httpCode is 200"
  },
  {
    code: "assert.equal(body.errcode, 0)",
    title: "assert return data errcode is 0"
  },
  {
    code: "assert.notEqual(status, 404)",
    title: "assert httpCode is not 404"
  },
  {
    code: "assert.notEqual(body.errcode, 40000)",
    title: "assert return data errcode is not 40000"
  },
  {
    code: 'assert.deepEqual(body, {"errcode": 0})',
    title: 'assert object body is {"errcode": 0}'
  },
  {
    code: 'assert.notDeepEqual(body, {"errcode": 0})',
    title: 'assert object body is not {"errcode": 0}'
  }
]
      ...
      <div className="interface-test postman">
        ...
        <div className="url">
          <InputGroup compact style={{ display: 'flex' }}>
            ...
            <Select value={case_env} style={{ flexBasis: 180, flexGrow: 1 }} onSelect={this.selectDomain} >
              ...
              <Option value="env setting" disabled style={{ cursor: 'pointer', color: '#2395f1' }}><Button type="primary" onClick={this.showEnvModal}>env setting</Button></Option>
            </Select>
          ...
          <Tooltip placement="bottom" title={(() => {
            if (hasPlugin) {
              ...
              return 'Please install cross-request plug-in'
            }
          })()}>
            <Button
              disabled={!hasPlugin}
            ...
            >{loading ? 'Cancle' : 'Send'}</Button>
          </Tooltip>

          <Tooltip placement="bottom" title={() => {
            return this.props.type === 'inter' ? 'Save to tests' : 'Update'
          }}><Button
            onClick={this.props.save}
            type="primary"
            style={{ marginLeft: 10 }}
          >{this.props.type === 'inter' ? 'Save' : 'Update'}</Button>
          </Tooltip>
        </div>
        ...
        <Collapse defaultActiveKey={['0', '1', '2', '3']} bordered={true}>
          <Panel header="PATH PARAMETERS" key="0" className={req_params.length === 0 ? 'hidden' : ''}>
            ...
            <Button style={{ display: 'none' }} type="primary" icon="plus" onClick={this.addPathParam}>Add Path Parameter</Button>
          </Panel>
          <Panel header="QUERY PARAMETERS" key="1" className={req_query.length === 0 ? 'hidden' : ''}>
            ...
            <Button style={{ display: 'none' }} type="primary" icon="plus" onClick={this.addQuery}>Add Query Parameter</Button>
          </Panel>
          <Panel header="HEADERS" key="2" className={req_headers.length === 0 ? 'hidden' : ''}>
            ...
            <Button style={{ display: 'none' }} type="primary" icon="plus" onClick={this.addHeader}>Add Header</Button>
          </Panel>
          <Panel
            header={
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Tooltip title="F9 Full Screen" >BODY(F9)</Tooltip>
              </div>
            }
            key="3"
          ...
          >
...

            {
              ...
              <div>
                ...
                <Button style={{ display: 'none' }} type="primary" icon="plus" onClick={this.addBody}>Add Form Parameter</Button>
              </div>
            }
            {
        ...
        <Tabs size="large" defaultActiveKey="res" className="response-tab"  >
          ...
          {this.props.type === 'case' ?
            <Tabs.TabPane className="response-test" tab={<Tooltip title="Test script，can assert return values, read documentation for more">Test</Tooltip>} key="test">
              <h3 style={{ margin: '5px' }}>
                &nbsp;Turn on/off:&nbsp;
                <Switch checked={this.state.enable_script} onChange={e => this.setState({ enable_script: e })} />
              </h3>
              <p style={{ margin: '10px' }}>Note：Test script only run on automated tests</p>
              <Row>
                <Col span="18">
                  <AceEditor
