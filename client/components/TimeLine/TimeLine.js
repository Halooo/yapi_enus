...
class TimeTree extends Component {
  ...
  render() {
    ...
    children.unshift(
      <Option value='' key='all' >Select all</Option>
    )
      ...
      data = data.map((item, i) => {
        ...
        return (<Timeline.Item dot={<Link to={`/user/profile/${item.uid}`}><Avatar src={`/api/user/avatar?uid=${item.uid}`} /></Link>} key={i}>
          ...
          <div style={{ padding: "10px 0 0 10px" }}>{interfaceDiff &&
            <Button onClick={() => this.openDiff(item.data)}>Changes Detail</Button>
          }</div>
        </Timeline.Item>);
      });
    } else {
    ...
    let pending = this.props.newsData.total <= this.props.curpage ? <a className="logbidden">All contents are displayed above</a> : <a className="loggetMore" onClick={this.getMore.bind(this)}>See more</a>;
    ...
    return (
      <section className="news-timeline">
        ...
        >
          <i>Note： green means new content，red means deleted</i>
        ...
        {this.props.type === 'project' &&
          <Row className="news-search">
            <Col span="3">Select Api to search：</Col>
            <Col span="10">
              <AutoComplete
